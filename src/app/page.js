...
export default function SetupPage() {
  ...
  async function generate() {
    ...
    const forName = encodeURIComponent(config.to);
    let url;
    try {
      const id = await createInvite(config);
      url = id
        ? `${origin}/undangan?untuk=${forName}&id=${id}`
        : `${origin}/undangan?untuk=${forName}&d=${encodeConfig(config)}`;
    } catch {
      url = `${origin}/undangan?untuk=${forName}&d=${encodeConfig(config)}`;
    }
    setLink(url);
    setCopied(false);
    setGenerating(false);
